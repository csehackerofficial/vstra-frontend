 ...
 * Features: User Management, Product CRUD with Percentages, and Banner Management.
 */
...
function renderUserTable(users) {
    ...
    if (!users || users.length === 0) {
        container.innerHTML = `<div class="text-center py-20 text-[11px] font-bold text-gray-300 uppercase tracking-widest">No Users Registered.</div>`;
        return;
    }
...
// ==========================================
// 🌟 STORE CATALOG (Edit & Discount Fix)
// ==========================================
...
function renderInventoryTable(products) {
    ...
    if (!products || products.length === 0) {
        container.innerHTML = `<div class="text-center py-20 text-[11px] font-bold text-gray-300 uppercase">Catalog is Empty.</div>`;
        return;
    }
    ...
    const rows = products.map(p => {
        ...
        <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors text-xs">
            ...
            <td class="py-4"><div class="font-bold text-gray-900 truncate max-w-[180px]">${p.name}</div></td>
            ...
            <td class="py-4">
                ...
                ${discount > 0 ? `<div class="text-[9px] text-blue-600 font-bold">-${discount}% OFF</div>` : ''}
            </td>
...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        if (res.ok) { toggleModal('productModal', false); loadInventory(); }
...
async function loadBanners() {
    ...
    try {
        ...
        if (allAdminBanners.length === 0) { list.innerHTML = `<div class="col-span-full text-center py-10 text-gray-300 uppercase text-[10px]">No Active Banners.</div>`; return; }
        list.innerHTML = allAdminBanners.map(b => `
            <div class="relative rounded-xl overflow-hidden border border-gray-100 shadow-sm h-36 group">
                <img src="${b.image_url}" class="w-full h-full object-cover">
                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                    <button onclick="editBanner(${b.id})" class="bg-white text-blue-600 p-2 rounded-full"><i class="ri-edit-box-line text-xl"></i></button>
                    <button onclick="deleteBanner(${b.id})" class="bg-white text-red-600 p-2 rounded-full"><i class="ri-delete-bin-line text-xl"></i></button>
                </div>
...
window.editBanner = (id) => {
    ...
    document.querySelector('#bannerModal h3').innerText = "Edit Homepage Banner";
...
window.saveBanner = async (e) => {
    ...
    try {
        ...
        const res = await fetch(url, { method, headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) });
        if (res.ok) { toggleModal('bannerModal', false); loadBanners(); }
...
async function loadSales() {
    ...
    try {
        ...
        if (allAdminSales.length === 0) { container.innerHTML = `<div class="text-center py-20 text-[11px] font-bold text-gray-300 uppercase">No Items Sold.</div>`; return; }
        const rows = allAdminSales.map(s => `
            <tr class="border-b border-gray-50 text-[11px]">
                <td class="py-4 px-6 font-mono">#${s.order_id}</td>
